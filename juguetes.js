...
async function obtenerProductos() { //defino una función asincrona (lee LINEA por LINEA)
  ...
  var juguetes = productos.filter(producto => producto.tipo == "Juguete"); //aplico un filtro para obtener el tipo de producto deseado
  //console.log(juguetes); //verificamos la correcta carga
  filtrar(juguetes); //ejecuto la función que va a filtrar e imprimir los productos
  imprimirHTML(juguetes); //ejecuto la funcion para que se imprima "la primera vez"
  juguetes.forEach(cadaProducto => { //por cada producto
